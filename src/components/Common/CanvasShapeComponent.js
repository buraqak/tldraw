...
import ShapecanvasHelp from "./ShapecanvasHelp";
...
const CanvasDraw = (props) => {
  ...
  function setFontColor(textColor) {
    ...
    if (obj && obj.get("type") == "i-text") {
    ...
    } else if (obj && obj.get("type") == "circle") {
      obj.set({ fill: textColor });
      ...
      if (
        activeMode != "insertText" &&
        activeMode != "pencil" &&
        activeMode != "circleObject"
      ) {
        draw(canvas);
      }
    }
  ...
  /* function setFontColor(textColor) {
    let obj = canvas.getActiveObject();
    if (obj) console.log("type:", obj.get("type"));
    if (obj && obj.get("type") == "i-text") {
      if (obj.isEditing) obj.setSelectionStyles({ fill: textColor });
      else {
        obj.set({ fill: textColor });
  ...
  useEffect(() => {
    if (activeMode != "circleObject" && canvas) {
      console.log("running:", circleObject);
      removeCanvasListener(canvas);
    }
    if (activeMode != 'selectObject' && canvas) {
  ...
  const uploadCanvasAndupdateSCORM = async (isAutoSave) => {
    ...
    if (canvasRef != null && canvasRef.current != null) {
      ...
      if (current_base64file != bas64file) {
        ...
        } else if (props.mode == "stage" || props.mode == "prod") {
  ...
  function saveCanvas() {
    ...
    downloadLink.setAttribute("download", "OSE_ShapeCanvas" + ".png");
            ...
            <ul className="d-grid justify-content-center align-items-center drawing-colors">
              ...
              <Tooltip title="Yellow" placement="bottom">
                <li
                  ...
                  onClick={() => (loading > 1 ? setFontColor("#FFD817") : null)}
                ></li>
              </Tooltip>
        ...
        <div className="d-grid align-items-center drawing-body">
          <div className="d-grid-column drawing-navi left">
            <ul className="d-grid justify-content-center align-items-center">
              ...
              <Tooltip title="Insert Circle" placement="left">
                <li
                  type="button"
                  id={"circleObject"}
                  className={
                    activeMode == "circleObject" ? "button active" : "button"
                  }
                  onClick={() => (loading > 1 ? circleObject(canvas) : null)}
                >
                  <img src="./img/drawing-circle.svg" disabled={loading == 1} />
                </li>
              </Tooltip>
